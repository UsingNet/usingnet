...
class SmsController extends Controller
{
    public function postSend(Request $request)
    {
        ...
        $params = $request->only('media_id', 'contact_ids', 'tpl');
        ...
        foreach ($contacts as $contact) {
            $content = sprintf('【%s】%s', $user->team->sms->signature, $params['tpl']);
            $method = 'send' . ucfirst($template->remark);
            $messageId = NewSms::$method($contact->phone, $content);
            Record::create([
                'phone' => $contact->phone,
    ...
    public function postCheck(Request $request)
    {
        ...
        $params['status'] = $params['action'] === 'reject' ? Media::STATUS_FAIL : Media::STATUS_SUCCESS;
