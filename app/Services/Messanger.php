...
class Messanger
{
    ...
    public static function send($data)
    {
        ...
        return self::getConnect()->emit($data);
    }
    ...
    public static function sms($data)
    {
        ...
        if ($data['direction'] == Message::DIRECTION_SEND) {
            ...
            if ($contact && $contact->phone) {
                $method = 'send' . ucfirst($data['package']['remark']);
                $messageId = NewSms::$method($contact->phone, $data['body']);
                Record::create([
                    'phone' => $contact->phone,
    ...
    {
        return new Connect(Connect::PUSH_SERVER);
    }

    public static function generateToken($type, $remote, $local, $self = [], $order_id = NULL)
    {
        $connect = new Connect(Connect::TOKEN_SERVER);

        $self['token_created_at'] = date('Y-m-d H:i:s');
        ...
        if ($type !== Message::TYPE_LISTENER) {
            $params['index']['remote'] = $remote;
            $params['condition'] = ['$or' => [[
                'from' =>  ['$in' => $params['index']['local']],
                'to' => ['$in' => $params['index']['remote']],
            ], [
                'to' => ['$in' => $params['index']['local']],
                'from' =>  ['$in' => $params['index']['remote']],
            ]]];
            if($order_id){
                //$params['condition']['package.order_id'] = $order_id;
            }
        }

        return $connect->generate(strtolower($type), $params);
    }
}
